...
def test_tc_fe_clients_054(client_page):
    """
    ...
    page.toggle_active_status(row_index=target_row, column_name=target_col)
    # page.toggle_active_status(  row_index=target_row, column_name=target_col)

    # Step 2 & 3: Check visibility of the confirmation controls
    assert page.is_component_visible(ModalLocators.CONFIRM_BUTTON), \
        "Confirmation dialog's CONFIRM button is not visible!"
        
    assert page.is_component_visible(ModalLocators.CANCEL_BUTTON), \
        "Confirmation dialog's CANCEL button is not visible!"

    # Clean up modal view if needed (optional)
    page.wait_for_and_click( ModalLocators.CANCEL_BUTTON)
