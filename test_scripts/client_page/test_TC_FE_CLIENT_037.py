...
def test_tc_fe_clients_037(client_page):
    """
    ...
    assert page.click_edit_button(), "Failed to click the Edit button!"

    # Step 2 & 3: Assert that Save button cannot be clicked without modifications
    assert not  page.is_component_clickable(  ModalLocators.SAVE_BUTTON), "Save button was clickable/enabled even though no changes were made in the Update Client modal!"
